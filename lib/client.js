...
class Client extends EventEmitter {
    ...
    _handleResponse(buffer) {
        ...
        if (this._cid) {
            ...
            if (pack.t.toLowerCase() === 'bindok') {
                return this._handleBindingConfirmationResponse(pack);
            }

            if (pack.t === 'subList') {
                return this._handleSubDevicesResponse(pack)
            }
     ...
     * @private
     */
    _handleHandshakeResponse(pack) {
        this._subCnt = pack.subCnt;
        console.log(this._subCnt)
        this._cid = pack.mac;
        this._sendBindRequest();
    }

    _requestSubDevices(mac) {
        this._sendRequest({
            t: 'subDev',
            mac: this._cid,
            i: 0
    ...
    _handleBindingConfirmationResponse(pack) {
        ...
        if (this._subCnt) {
            ...
            this._handleSubDevicesResponse()
        }
    }

    _handleSubDevicesResponse(pack) {
        if (pack) {
            const devices = pack.list;
            if (!devices.length) {
                console.log('no sub devices found')
                return;
            }
            devices.forEach(device => {
                console.log('found sub device', device)
